...
def  k_fold_cv(folds,dataset):
    """
    Splits the dataset into  train and test split by cross validation
    If there is a separate testfile, it can be specified in testfile
    If a subset of features is desired, this can be specifed with featureinds; defaults to all
    Assumes output variable is the last variable
    """
...
def Normalize(dataset,train_index,test_index,testdataset=None, featureoffset=None, outputfirst=None):
    featureend = dataset.shape[1]-1
    outputlocation = featureend    
    if featureoffset is None:
        featureoffset = 0
    ...
    Xtrain = dataset[train_index,featureoffset:featureend]
    ytrain = dataset[train_index,outputlocation]
    Xtest = dataset[test_index,featureoffset:featureend]
    ytest = dataset[test_index,outputlocation]
